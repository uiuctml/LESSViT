...
from .enmap import S2C_MEAN, S2C_STD, S2C_WV, SELECTED_CHANNEL_IDX_B, SELECTED_CHANNEL_IDX_A, SELECTED_CHANNEL_IDX_B_60, SELECTED_CHANNEL_IDX_B_30, SELECTED_CHANNEL_IDX_B_10


class EnMAPBDForetDataset(Dataset):
    """PyTorch dataset for EnMAP-BDForet samples."""

    foreground_classes = [0, 1, 2, 3, 4, 5, 7, 9, 10, 11, 12, 14, 16]
    ...
    def __init__(self, root: str, split: str, transform, gen_task: Optional[str] = None) -> None:
        ...
        """
        self.root = os.path.join(root, "enmap_bdforet")
        self.split_file = os.path.join(root, "splits", "enmap_bdforet", f"{split}.txt")
        self.split = split
    ...
    def __getitem__(self, index: int) -> dict[str, object]:
        ...
        # optical = optical[:181, :, :]
        # optical_channel_wv = [optical_channel_wv[i] for i in range(181)]
